...
async def start_and_stop(
    ...
    async with websockets.connect(
        ...
        # XXX: Here we demonstrate the memory error in the
        # GetDataSubarray-message

        # First we access too many rows (zeroth-axis). This returns a protocol
        # exception. No crash yet.
        records = await get_data_subarray(
            ws,
            epc_uri,
            pir[sorted(pir)[0]],
